...
class LargeFile:
    ...
    def select_directory_and_show_large_files(self):
        ...
        if large_files:
            ...
            self.status_label.config(text="No large files found in the directory.")
    ...
    def delete_selected_and_show_status(self):
        ...
        if selected_files:
            ...
            # Remove the selected files from the list after deletion
            self.selected_files_var = [(file, size, var) for file, size, var in self.selected_files_var if var.get() == 0]
