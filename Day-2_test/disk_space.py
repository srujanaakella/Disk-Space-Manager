...
class DiskSpaceVisualizer:
    def __init__(self):
        ...
        self.title_label = tk.Label(self.root, text="Disk Space Information", font=self.title_font)
    ...
    def display(self):
        for drive, space_info in self.disk_space_info.items():
            ...
            label_drive = ttk.Label(frame, text=f"Drive {drive}:", font=self.space_font)
            ...
            label_total = tk.Label(frame, text=f"Total: {self._format_bytes(total_space)}", font=self.space_font)
            label_total.pack(side=tk.LEFT, padx=10)

            label_used = tk.Label(frame, text=f"Used: {self._format_bytes(used_space)}", font=self.space_font)
            label_used.pack(side=tk.LEFT, padx=10)

            label_free = tk.Label(frame, text=f"Free: {self._format_bytes(free_space)}", font=self.space_font)
