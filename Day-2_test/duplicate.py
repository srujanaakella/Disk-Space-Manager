...
def get_avhash(file_path):
    try:
        hash_obj = imagehash.average_hash(imagehash.open_img(file_path))
        return str(hash_obj)
...
def find_duplicate_files(directory):
    ...
    with ThreadPoolExecutor() as executor:
        futures = {executor.submit(get_file_hash, os.path.join(dirpath, filename)): filename for dirpath, _, filenames in os.walk(directory) for filename in filenames}

    for future in futures:
        file_hash = future.result()
        filename = futures[future]
        av_hash = get_avhash(os.path.join(directory, filename))
        if file_hash in file_hashes:
            duplicate_files.append((os.path.basename(file_hashes[file_hash]), os.path.basename(filename), av_hash))
        else:
...
class DuplicateFilesGUI(tk.Toplevel):
    ...
    def find_duplicates(self):
        ...
        if not self.duplicate_pairs:
            ...
            self.checkboxes = []
            for i, (file1, file2, av_hash) in enumerate(self.duplicate_pairs, start=1):
                var = tk.IntVar()
