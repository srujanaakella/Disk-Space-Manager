...
class DuplicateFilesGUI(tk.Toplevel):
    ...
    def find_duplicates(self):
        ...
        # Divide the directory into chunks
        chunks = [os.path.join(directory, name) for name in os.listdir(directory)]

        with ProcessPoolExecutor() as executor:
            # Process chunks concurrently
            for duplicate_files in executor.map(find_duplicate_files, chunks):
                self.duplicate_pairs.extend(duplicate_files)
