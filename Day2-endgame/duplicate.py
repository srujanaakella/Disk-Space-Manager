...
class DuplicateFilesGUI(tk.Toplevel):
    def __init__(self):
        ...
        self.select_all_checkbutton = tk.Checkbutton(self, text="Select All", variable=self.select_all_var, command=self.toggle_select_all)
    ...
    def find_duplicates(self):
        ...
        if not self.duplicate_pairs:
            ...
            for i, (file1, file2) in enumerate(self.duplicate_pairs, start=1):
                var = tk.IntVar()
                checkbox = tk.Checkbutton(self.scrollable_frame, text=f"{file1} -- {file2}", variable=var, onvalue=1, offvalue=0)
